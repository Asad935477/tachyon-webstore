...
import { Textarea } from "@tachyon-webstore/ui/components/textarea";
...
function InputGroup({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			role="group"
			className={cn(
				"group/input-group relative flex h-9 w-full min-w-0 items-center rounded-lg border border-input bg-background outline-none transition-[color,box-shadow] has-[>textarea]:h-auto dark:bg-input/30",
...
const inputGroupAddonVariants = cva(
	"flex h-auto cursor-text select-none items-center justify-center gap-2 py-1.5 text-sm font-medium text-muted-foreground group-data-[disabled=true]/input-group:opacity-50 [&>kbd]:rounded-md [&>svg:not([class*='size-'])]:size-4",
	{
		variants: {
			align: {
				"inline-start":
...
const inputGroupButtonVariants = cva(
	"flex items-center gap-2 text-sm shadow-none",
	{
		variants: {
			size: {
				xs: "h-7 gap-1 rounded-md px-2 [&>svg:not([class*='size-'])]:size-3.5",
				sm: "h-8 gap-1 rounded-md px-2.5",
				"icon-xs": "size-7 rounded-md p-0 has-[>svg]:p-0",
...
function InputGroupInput({
	...
	return (
		<Input
			...
			className={cn(
				"h-full flex-1 rounded-lg border-0 bg-transparent px-3 shadow-none ring-0 focus-visible:ring-0 disabled:bg-transparent aria-invalid:ring-0 dark:bg-transparent dark:disabled:bg-transparent",
				className,
			)}
			{...props}
		/>
